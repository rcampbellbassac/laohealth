...
export default new Vuex.Store({
  ...
  actions: {
    async initdata({ commit }) {
      localforage.getItem('LaoHealth').then((localstoragedata) => {
        if (localstoragedata != null) {
          ...
          fetch('https://laoadventist-media.s3-us-west-2.amazonaws.com/LaoHealthBooks.json')
            .then((response) => (response.json()))
            .then(async (responseJSON) => {
              await localforage.setItem('LaoHealth', responseJSON)
